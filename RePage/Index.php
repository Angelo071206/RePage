...
.nav-buttons .dropdown-content {
    ...
    min-width: 150px;
    padding: 5px 0;
    z-index: 999; /* Aggiungi un alto valore di z-index per evitare la sovrapposizione */
}
...
<div class="content">
    ...
    <div class="chat-list">
        ...
        if (isset($_SESSION['user'])) {
            if (isset($_SESSION['user']['ID_Utente'])) {
                ...
                if ($stmt = $conn->prepare($sql_chat)) {
                    ...
                    echo "<p>Errore nella preparazione della query per le chat.</p>";
                }
            } else {
                echo "<p class='no-chats'>Errore: utente non loggato correttamente.</p>";
            }
        } else {
...
<script>
function apriPopup(titolo, autore, isbn, condizione, descrizione, imgPath) {
...
function contatta() {
    // Funzione per inviare un messaggio di contatto
    alert('Funzione di contatto non ancora implementata!');
}
</script>
